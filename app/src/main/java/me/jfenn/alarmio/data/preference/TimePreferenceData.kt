 ...
 */
class TimePreferenceData(private val preference: PreferenceData, name: Int) :
    ...
    override fun onClick(holder: CustomPreferenceData.ViewHolder) {
        val dialog = run {
            var seconds =
                TimeUnit.MILLISECONDS.toSeconds(preference.getValue<Long>(holder.context)).toInt()
        ...
        dialog.setListener { hours, minutes, seconds ->
            run {
                seconds + TimeUnit.HOURS.toSeconds(hours.toLong())
                    .toInt() + TimeUnit.MINUTES.toSeconds(minutes.toLong()).toInt()
            }.let { totalSeconds ->
                preference.setValue(
                    holder.context,
                    TimeUnit.SECONDS.toMillis(totalSeconds.toLong())
                )
